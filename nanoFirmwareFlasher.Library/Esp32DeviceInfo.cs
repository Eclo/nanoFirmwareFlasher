...
namespace nanoFramework.Tools.FirmwareFlasher
{
    ...
    public class Esp32DeviceInfo
    {
        ...
        public override string ToString()
        {
            ...
            deviceInfo.Append($"{ChipType}");

            // add details, if relevant
            if (ChipType != ChipName)
            {
                deviceInfo.AppendLine($" ({ChipName})");
            }
            else
            {
        ...
        private string GetFlashManufacturer()
        {
            ...
            if (match.Success)
            {
                string mfgLine = match.Groups["mgfid"].ToString();

                // find 1st _ID
                int index = mfgLine.IndexOf("_ID");

                return mfgLine.Substring(0, index);
            }

            return "";
        }

        private string GetFlashDeviceId()
        {
            int mfgLIndex;
            string mfgId;
            ...
            if (match.Success)
            {
                ...
                // find 1st _ID
                mfgLIndex = mfgLine.IndexOf("_ID");

                // get manufacturer ID (including _) for replacement
                mfgId = mfgLine.Substring(0, mfgLIndex + 1);

                // now find device ID
                match = Regex.Match(FlashChipIds.Substring(mfgLIndex), $"({mfgId})(?<deviceid>.*0x{FlashDeviceId:X})");
        ...
        //
        // this list was taken from https://github.com/flashrom/flashrom/blob/staging/flashchips.h
        //
        private string FlashChipIds = @"
/*
