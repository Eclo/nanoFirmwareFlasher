...
namespace nanoFirmwareFlasher.Tests
{
    ...
    public class McumgrClientProtocolTests
    {
        ...
        // -----------------------------------------------------------------------
        // img_state_write_verify equivalents — image state (test / confirm)
        //
        // Zephyr's img_state_write_verify() calls img_mgmt_client_state_write() and
        // checks the encoded CBOR matches expectations.  These tests verify
        // EncodeImageStateWrite mirrors newtmgr's ImageStateWriteReq exactly:
        //   • "hash"    present when hash is non-null, encoded as byte string
        //   • "confirm" ALWAYS present (no omitempty, even when false)
        // -----------------------------------------------------------------------

        [TestMethod]
        public void ImageStateWrite_TestWithHash_ConfirmAlwaysPresent()
        {
            // mirrors img_state_write_verify: sets pending on secondary slot using hash.
            // newtmgr sends { hash: <32 bytes>, confirm: false }.
            byte[] hash = new byte[32];
            for (int i = 0; i < 32; i++) hash[i] = (byte)(i + 32);

            byte[] cbor = McumgrClient.EncodeImageStateWrite(hash, confirm: false);

            Assert.IsTrue(HasField(cbor, "hash"),    "test-with-hash must encode 'hash' field");
            Assert.IsTrue(HasField(cbor, "confirm"), "'confirm' must be present even when false (no omitempty)");
        }

        [TestMethod]
        public void ImageStateWrite_TestWithHash_ConfirmValueIsFalse()
        {
            byte[] hash = new byte[32];
            byte[] cbor = McumgrClient.EncodeImageStateWrite(hash, confirm: false);

            bool confirmVal = true; // sentinel
            var r = new CborReader(cbor, CborConformanceMode.Lax);
            r.ReadStartMap();
            while (r.PeekState() != CborReaderState.EndMap)
            {
                string key = r.ReadTextString();
                if (key == "confirm") confirmVal = r.ReadBoolean();
                else                  r.SkipValue();
            }

            Assert.IsFalse(confirmVal, "confirm must be false in test mode");
        }

        [TestMethod]
        public void ImageStateWrite_ConfirmWithHash_BothFieldsPresent()
        {
            // mirrors img_state_write_verify: permanently confirms secondary slot image.
            // newtmgr sends { hash: <32 bytes>, confirm: true }.
            byte[] hash = new byte[32];
            for (int i = 0; i < 32; i++) hash[i] = (byte)(i + 64);

            byte[] cbor = McumgrClient.EncodeImageStateWrite(hash, confirm: true);

            Assert.IsTrue(HasField(cbor, "hash"),    "'hash' must be present in confirm-with-hash request");
            Assert.IsTrue(HasField(cbor, "confirm"), "'confirm' must be present in confirm-with-hash request");
        }

        [TestMethod]
        public void ImageStateWrite_ConfirmWithHash_HashMatchesInput()
        {
            byte[] hash = new byte[32];
            for (int i = 0; i < 32; i++) hash[i] = (byte)(i + 64);
            ...
            byte[] decoded = ReadByteStringField(cbor, "hash");
            CollectionAssert.AreEqual(hash, decoded, "hash field must carry the exact hash bytes provided");
        }

        [TestMethod]
        public void ImageStateWrite_ConfirmWithoutHash_OnlyConfirmTrue()
        {
            // mirrors confirming the currently-running image (no hash needed):
            // newtmgr sends { confirm: true }
            byte[] cbor = McumgrClient.EncodeImageStateWrite(hash: null, confirm: true);

            Assert.IsFalse(HasField(cbor, "hash"),  "confirm without hash must not encode 'hash' field");
            Assert.IsTrue(HasField(cbor, "confirm"), "'confirm' must be present");

            var r = new CborReader(cbor, CborConformanceMode.Lax);
            r.ReadStartMap();
            r.ReadTextString(); // "confirm"
            bool val = r.ReadBoolean();
            Assert.IsTrue(val, "'confirm' must be true");
        }

        [TestMethod]
        public void ImageStateWrite_TestWithoutHash_OnlyConfirmFalse()
        {
            // newtmgr sends { confirm: false } (testing running image with no specific hash).
            byte[] cbor = McumgrClient.EncodeImageStateWrite(hash: null, confirm: false);

            Assert.IsFalse(HasField(cbor, "hash"),  "test without hash must not encode 'hash' field");
            Assert.IsTrue(HasField(cbor, "confirm"), "'confirm' must be present even when false");
